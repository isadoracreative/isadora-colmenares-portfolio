...
async function main() {
  ...
  for (const filePath of files) {
    ...
    entries.push(result);
    savedBytes += Math.max(0, result.originalSize - result.webpSize);
    console.log(
      `${path.basename(filePath)} → ${path.basename(result.webpSrc)} (${formatBytes(result.originalSize)} → ${formatBytes(result.webpSize)})`,
    );
  }
